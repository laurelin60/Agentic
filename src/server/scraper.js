...
class BrowserInterface {
    ...
    async getPageContents() {
        ...
        const textObject = await this.transformToTextObject(document.documentElement);
    ...
    async checkVisibility() {
        ...
        while (attempts < 3) {
            attempts++;
            try {
                await this.driver.executeScript(async (elements) => {
                    console.log("Processing visibility for " + elements.length + " elements");
                    let res = await Promise.all(elements.map(async element => {
                        try {
                            if (element) {
                                const style = window.getComputedStyle(element);
                                const good = element.children.length > 0 || !style || (style && style.display !== 'none' && style.visibility !== 'hidden' && element.offsetWidth > 0 && element.offsetHeight > 0);
                                if (good) {
                                    element.classList.add("epic-good-element-thingy-yay");
                                    if (getEventListeners(element).click) {
                                        element.classList.add("epic-good-element-thingy-clickable");
                                    }
                                }
                                else {
                                    //element.textContent = "";
                    ...
                    return res;
                }, await this.driver.findElements(By.css('*'))); // Make sure elements are passed as arguments
                break;
            }
            catch (e) {
                await new Promise(resolve => setTimeout(resolve, 1500));
    ...
    async clickElement(selector) {
        await this.driver.executeScript(async (selector, driver) => {
            let element = document.querySelector(selector);
            if (element && element.click) {
                element.click();
            }
        }, selector, this.driver);
    }
    ...
    isInteractable(element) {
        ...
        return interactableTags.includes(element.tagName) || (element.classList && element.classList.contains("epic-good-element-thingy-clickable"));
    }
    ...
    async transformToTextObject(element) {
        const nonContentTags = ['SCRIPT', 'STYLE', 'NOSCRIPT', 'BR', 'HR', 'HEAD', 'LINK', 'META', 'TITLE'];
        const minimalAttributes = ['href', 'name', 'type', 'placeholder', 'value', 'aria-label'];
        ...
        if (elementDetails.t && (elementDetails.t.length > 15000 || (elementDetails.t.length >= 5 && elementDetails.t.substr(0, 5) == "html:"))) {
            return undefined;
        }
        ...
        if (!elementDetails.i && !elementDetails.t && elementDetails.c && elementDetails.c.length == 1 && hasSignificantChildren) {
            let childDetails = elementDetails.c[0];
            return Object.keys(childDetails).length > 0 ? childDetails : undefined;
        }
    
        return Object.keys(elementDetails).length > 0 ? elementDetails : undefined;
    }
}
