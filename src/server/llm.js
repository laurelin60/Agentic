...
const { webSearch, BrowserInterface } = require("./scraper.js");
...
class LLMInterface {
    ...
    constructor() {
        ...
        try {
            LLMInterface.API_KEY = fs.readFileSync("geminikey.txt", "utf8");
        } catch (e) {
            console.log(chalk.redBright(`Error reading API key from geminikey.txt
            Please create a file called geminikey.txt in the testing folder and paste your API key in there.
        ...
        this.model = this.genAI.getGenerativeModel({ model: LLMInterface.MODEL_NAME });
        this.prompt = {
    ...
    addUserMessage(message) {
        ...
        this.currMessage += "<UserToAssistant>" + message + "</UserToAssistant>\n";
    }
    ...
    async promptLLM() {
        ...
        while (attempts < 3) {
            try {
                ...
                console.log(chalk.redBright(`LLM had a stroke (${attempts}/3)`));
                if (attempts == 3) {
    ...
    async promptLLMVision() {
        ...
        const result = await this.model.generateContent({
            contents: [{
                role: "user",
                parts: [{ text: this.getTextPrompt() }, { inlineData: { mimeType: "image/png", data: img64 } }]
            }],
            generationConfig: LLMInterface.generationConfig,
            safetySettings: LLMInterface.safetySettings
        });
    ...
    async runUserMessage() {
        ...
        if (!response.trim().endsWith(">")) {
            ...
            this.addSystemMessage("Oops, some of your content was not included in tags (such as AssistantToUser) and was not sent to the user. Please try again.");
        ...
        for (const action of actions) {
            if (action.type === "CallToSystem") {
                ...
                if (command === "WEBSEARCH" || command === "NAVIGATE" || command === "SEARCH" || command === "CLICK" || command === "ENTER") {
                    await this.runUserMessage();
                    systemCalled = true;
                    break;
                }
        ...
        if (!systemCalled) {
            ...
            if (!AssistantToUserFound) {
                this.addSystemMessage("You did not include a AssistantToUser in your response. Please send one immediately.");
                await this.runUserMessage();
            }
        }
    }
    ...
    displayMessageToUser(message) {
        console.log(chalk.green("[AssistantToUser]"), chalk.greenBright(message));
    }

    async clickElement(id) {
        let selector = this.browser.getSelectorFromIndex(id);
        if (selector === null) {
            console.log(chalk.redBright(`Could not click element ${id} because it doesn't exist`));
            this.addSystemMessage(`Could not click element ${id} because it doesn't exist`);
            return;
        }
        if (!(this.browser.isSelectorInteractable(selector))) {
            console.log(chalk.redBright(`Could not interact with element ${id} because it's not interactable`));
            this.addSystemMessage(`Could not interact with element ${id} because it's not interactable! You can try again, this time check the interactable tag before attempting to interact.`);
            return;
        }
        ...
        this.addSystemMessage(`Clicked element ${id}, updated website data is in "CURRENT URL" and "WEBSITE DATA". Note that you may or may not have made a mistake and clicked something that didn't do anything.`);
    }
    ...
    async enterText(id, text) {
        ...
        let selector = this.browser.getSelectorFromIndex(id);
        if (selector === null) {
            console.log(chalk.redBright(`Could not interact with element ${id} because it doesn't exist`));
            this.addSystemMessage(`Could not interact with element ${id} because it doesn't exist`);
            return;
        }
        if (!(this.browser.isSelectorInteractable(selector))) {
            console.log(chalk.redBright(`Could not interact with element ${id} because it's not interactable`));
        ...
        console.log(chalk.gray(`Setting text of element ${id} [${selector}] to "${text}"`));
        await this.browser.enterText(selector, text);
        await new Promise(t => setTimeout(t, 100));
        let results = await this.browser.getPageContents();
        this.prompt.currentUrl = this.browser.driver.getCurrentUrl();
        this.prompt.websiteData = Utils.compactJsonStringify(results);
        this.addSystemMessage(`Attempted to enter text into element ${id}, updated website data is in "CURRENT URL" and "WEBSITE DATA". Note that you may or may not have made a mistake and clicked something that didn't do anything.`);
    ...
    async webSearch(query) {
        ...
        this.addSystemMessage("Web search completed, results are in \"LAST SEARCH\".");
    }
    ...
    async navigate(url) {
        ...
        this.addSystemMessage("Navigation completed, website data is in \"CURRENT URL\" and \"WEBSITE DATA\".");
    }
                ...
                switch (command) {
                    case "CLICK":
                        if (!this.isValidId(parts[1])) {
                            throw new Error(`Invalid ID for CLICK: ${parts[1]}`);
                        }
                        break;
                    case "ENTER":
                        if (!this.isValidId(parts[1])) {
                            throw new Error(`Invalid ID for ENTER: ${parts[1]}`);
                        }
                        break;
                    case "WEBSEARCH": {
                        const query = parts.slice(1).join(", ");
                        if (!this.isValidWebSearchQuery(query)) {
                            throw new Error(`Invalid query for WEBSEARCH: ${query}`);
                        }
                    }
                        break;
                    case "SEARCH": // in case llm has a stroke
                    {
                        ...
                        break;
                    case "NAVIGATE":
                        if (!this.isValidUrl(this.removeSurroundingQuotes(parts[1]))) {
                            throw new Error(`Invalid URL for NAVIGATE: ${parts[1]}`);
                        }
                        break;
                    default:
    ...
    async handleSystemCall(call) {
        ...
        switch (command) {
            case "CLICK":
                ...
                break;
            case "ENTER":
                await this.enterText(parseInt(parts[1]), parts.slice(2).join(", "));
                break;
            case "WEBSEARCH":
